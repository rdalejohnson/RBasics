...
dat <- data.frame(
  "name" = c("abe", "charles", "margaret"),
  "gender" = c("male", "male", "female"),
  "zipcode" = c("50222", "30222", "70221"),
  "month1" = c(0, 5, 2),
  "month2" = c(5, 3, 5),
  "month3" = c(0, 25, 2),
  "month4" = c(1, 4, 17),
...
drop.cols <- c('gender', 'zipcode')
  
library(dplyr)
library(reshape2)
library(ggplot2)
#Convert to long format
d = melt(select(dat, -one_of(drop.cols)), id.vars = "name")

ggplot(data = d,
       mapping = aes(x = name, y = value, fill = variable)) + 
